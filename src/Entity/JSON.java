	...
	public Map<QuestionStrength, List<Question>> loadQuestions(String externalPath) {
		...
		try {
			...
			while (quesIterator.hasNext()) {
				...
				if (reNumber) {
					qnum = autoQuestionNumber;
					autoQuestionNumber++;
				} // for renumbering (set renumber true)

				Question toAdd = new Question(qnum, getQuestionStrength((long) q.get("difficulty")),
						(String) q.get("text"), (boolean) q.get("isMultipleChoice"), answers,
						Team.valueOf((String) q.get("team")), tags);
	...
	public void saveQuestions(Map<QuestionStrength, List<Question>> questions) {
		...
		for (Map.Entry<QuestionStrength, List<Question>> list : questions.entrySet()) {
			...
			for (Question q : list.getValue()) {
				...
				// get questions properties from object to json objects and
				// arrays
