...
public class SysParam {
	...
	/**
	 * Action Listener - get the source of the button and does the required
	 * action
	 *
	 * @param e
	 */
	...
	private void restoreDefaults() {
		...
		/*
		 * //add style to alert DialogPane dialogPane = alert.getDialogPane();
		 * dialogPane.getStylesheets().add(
		 * getClass().getResource("/View/Admin/myDialog.css").toExternalForm());
	...
	private boolean checkRules() {
		...
		if (Validators.string2Double(spinMinLuck.getValue().toString()) > Validators
				.string2Double(spinMaxLuck.getValue().toString())) {
			errorLabelControl("Min luck can't be greater than Max luck", true);
			return false;
		}

		// check if prices are logical
		if (Validators.string2Double(spinEasyPrice.getValue().toString()) > Validators
				.string2Double(spinMedPrice.getValue().toString())
				|| Validators.string2Double(spinMedPrice.getValue().toString()) > Validators
						.string2Double(spinHardPrivce.getValue().toString())) {
	...
	// spinner properties => min="50000" max="10000000" initialValue="100000"
	// amountToStepBy="10000"
}
